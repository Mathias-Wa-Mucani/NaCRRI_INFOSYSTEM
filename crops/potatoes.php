...
<h1 style="background-color:lightgreen; width:100%; postion:fixed; padding-left:1rem; font-size:1.4rem">POTATOES RESEARCHES</h1>
<h3>This the place where you get to know all about the potatoes and what it takes to become a good farmer in potatoes</h3>
...
if ($checkrows->num_rows>0){
    while($row = $checkrows->fetch_assoc()){
        ...
        if ($row['date']!="" && $row['potatoes']!='' && $row['AdminName']!="") {
            
        
            echo nl2br("<div class='post'><b style=''>Posted at  ". $row["date"]." by "."<i style='color:blue;'>".$row['AdminName']."</i>"."</b>"."</div>"."<br>".wordwrap($row["potatoes"], 150));
            echo "<br>";
    ...
    echo "No research so far posted about potatoes.....";
