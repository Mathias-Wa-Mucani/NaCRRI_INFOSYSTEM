...
<h1 style="background-color:lightgreen; width:100%; postion:fixed; padding-left:1rem; font-size:1.4rem">OIL PALM RESEARCHES</h1>
<h3>This the place where you get to know all about the Oil Palms and what it takes to become a good farmer in Oil Palms</h3>
...
$check= "SELECT * FROM cropsresearch order by id desc"; //use distinct to select only unique records 
$checkrows=$connect->query($check);

if ($checkrows->num_rows>0){
    while($row = $checkrows->fetch_assoc()){
        
        if ($row['date']!="" && $row['oilpalm']!='' && $row['AdminName']!="") {
            
        
            echo nl2br("<div class='post'><b style=''>Posted at ". $row["date"]." by "."<i style='color:blue;'>".$row['AdminName']."</i>"."</b>"."</div>"."<br>".wordwrap($row["oilpalm"], 150));
        echo "<br>";
    ...
    echo "No research so far posted about Oil Palm.....";
