...
<h1 style="background-color:lightgreen; width:100%; postion:fixed; padding-left:1rem; font-size:1.6rem">RICE RESEARCHES</h1>
<h3>This the place where you get to know all about the rice and what it takes to become a good farmer in rice</h3>
...
if ($checkrows->num_rows>0){
    while($row = $checkrows->fetch_assoc()){
        ...
        if ($row['date']!="" && $row['rice']!='' && $row['AdminName']!="") {
            
        
        echo nl2br("<div class='post'><b style=''>Posted at ". $row["date"]." by "."<i style='color:blue;'>".$row['AdminName']."</i>"."</b>"."</div>"."<br>".wordwrap($row["rice"], 150));
        echo "<br>";
    ...
    echo "No research so far posted about rice.....";
